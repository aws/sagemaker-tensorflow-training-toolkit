...
def required_versions(tf_version):
    if tf_version == '1.4.1':
        return ['tensorflow-serving-api==1.4.0', 'tensorflow-tensorboard==0.4.0', 'tensorflow==1.4.1']
    elif tf_version == '1.5.0':
        return ['tensorflow-serving-api==1.5.0', 'tensorflow-tensorboard==1.5.1', 'tensorflow==1.5.0']
    else:
...
def test_framework_versions(docker_image, required_versions):
    with Container(docker_image) as c:
        ...
        result = sorted(filter(lambda x: 'tensorflow' in x, lines))
